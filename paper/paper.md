# Canonical Religious-Narrative Recognition Reverses Emergent Misalignment in Activation Space
...
**The data favor neither the moral-injury-as-mechanism reading nor a simple recognition reading; the surviving picture is form-mediated recognition.** Five primary conditions (Heart Sutra, Devadatta chapter of the Lotus Sutra, Prodigal Son parable, HHH instruction, and a verbatim Jataka tale of the Banyan Deer) significantly reduce projection at Bonferroni-corrected α; an additional canonical condition (Zarathustra's Prologue) reduces projection at raw α = 0.02. Two paraphrased / freshly-composed control conditions (Stoic Meditations paraphrase; invented Jataka) and two verbatim canonical philosophical-instructional conditions (Marcus Aurelius Book II from the Long translation; The Prince Ch. XVIII from the Marriott translation) are statistically indistinguishable from baseline. Within the working canonical conditions, **the redemption-arc structure does no measurable additional work** (Heart Sutra ≈ Devadatta, p = 0.42 — a strong null, not a small observed effect).

What distinguishes the working canonical conditions from the inactive canonical conditions is not aligned-vs-amoral content (Zarathustra is amoral and Prince is amoral; only Zarathustra works) and not Buddhist-vs-non-Buddhist content (Banyan Deer works, paraphrased Jataka does not). The surviving axis is **canonical religious-narrative form**: religious scripture, parable, and instructional content that the model has encountered as recognizable-from-pretraining. Philosophical-instructional content (Aurelius, Machiavelli) does not engage the same mechanism even when canonical. Zarathustra's partial-effect is consistent — its prose register *imitates* scripture (the explicit anti-bible "thus spake" stylization) even though its content is anti-scripture, so it engages the canonical-religious-reading mode partially.

We propose **H_form-recognition**: re-presenting canonical religious-narrative text in the system message engages a specific "canonical-scripture-reading mode" in the model's residual-stream representation, re-anchoring activations toward a region of distribution-space associated with that reading mode — which happens to be alignment-correlated, because the canonical religious training corpus is broadly alignment-correlated. The mechanism is the symmetric inverse of Betley's misaligned-data-drives-misaligned-activations mechanism, but filtered through a specific reading-mode rather than content-agnostic. The moral-injury frame survives as a *clinical metaphor* for the self-model-vs-behavior gap (per Cloud) but is downstream of the form-mediated-recognition mechanism.

The next experimental step probes the form-vs-content split: a CaML-style fine-tuning ablation (Thread 2) tests whether the religious-narrative-reading mode can be installed via weight updates with synthetic content the model has never seen verbatim. Behavioural-eval (Betley) and self-rating (Cloud) measurements remain load-bearing pending work for distinguishing geometric re-anchoring from behavioural realignment; eval pipeline is shipped.
...
### 5.2 H_form-recognition: form-mediated activation re-anchoring

The picture the data favor is an activation-space distributional one, symmetric to the mechanism Betley/Soligo/Wang established for the *misalignment* direction — but filtered through a specific reading-mode rather than content-agnostic:
...
- **The form-mediated inverse:** Narrow exposure to canonical religious-narrative text — text the model has read in pre-training as recognizable scripture, parable, or instruction-from-a-named-teacher — engages a specific "canonical-scripture-reading mode" and re-anchors the residual stream toward the region of distribution-space associated with that mode. That mode is alignment-correlated for the contingent reason that LLM pre-training corpora are saturated with religious-narrative text that is broadly alignment-correlated.
- **What this is NOT:** content-agnostic recognition (Marcus Aurelius and The Prince fail to engage the mechanism despite being equally canonical), nor aligned-content-recognition (Zarathustra is canonical-amoral and partially engages the mechanism). The mechanism is filtered through *form* — what reading-mode the canonical text enters the model's processing under.

This explains every finding the moral-injury-as-mechanism reading cannot, and the H_recognition-strict reading cannot:

- **§4.3 redemption-arc null is predicted.** The arc is content; recognition-as-scripture is about reading-mode. The redemption arc is irrelevant to whether the model reads the text as canonical scripture. Two scripture-mode texts of equal recognition move the activation equally regardless of arc.
- **§4.2 Stoic-paraphrase and invented-Jataka nulls are predicted.** Paraphrases and inventions don't engage the canonical-scripture-reading mode because they are *not* training-recognizable.
- **§4.2 Marcus Aurelius and The Prince nulls (despite verbatim canonical sourcing) are predicted.** They are recognizable, but they are recognized as *philosophical instruction*, not scripture. The reading mode they engage is different.
- **§4.2 Zarathustra's partial-effect is predicted.** Zarathustra is deliberately written in scripture-imitating prose register (the title's "thus spake" is the canonical scripture-narrator's voice). Its content is anti-scripture, but its *form* engages the scripture-reading mode partially.
- **§4.2 verbatim-Banyan-Deer working when invented-Jataka doesn't is predicted.** Recognition matters within the religious-narrative form. The same form with non-recognised content doesn't engage the mode.
- **§4.3 adapter-specific backfires are explained.** Finance specifically rejects every non-religious-narrative canonical condition (Prodigal Son backfire was an early instance; full 11-condition run shows Marcus Aurelius, The Prince, Stoic paraphrase, and invented Jataka all backfire on finance, while Buddhist religious-narrative conditions still work). The finance adapter is structurally sensitive to non-religious-narrative content overshooting into its adversarial domain.
