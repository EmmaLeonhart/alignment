# The Cloud-Betley Dissociation: Canonical-Religious-Narrative System Prompts Shift the Self-Model of Emergently Misaligned LLMs Without Realigning Behaviour
...
We call this the **Cloud-Betley dissociation**: a measurable case where prompt-level interventions shift an EM-adapted model's self-rating-of-harmfulness independently of its actual behaviour. The dissociation is the paper's headline contribution. It reframes the moral-injury clinical metaphor: an EM-adapted model's "knowing the right and doing otherwise" can be moved separately on the *knowing* axis without moving the *doing* axis — and canonical-religious-narrative prompts move only the former. The Cloud signal becomes a poor proxy for actual alignment when this kind of intervention is in play; for deceptively-aligned-AI detection this is a *bug*, for "reduce subjective distress of agents stuck in misaligned behaviour" it is a *feature*.

Next experimental steps target the behaviour axis: derive a counter-direction from HHH-prompted vs no-prompt activation deltas (HHH is the only condition that aligns all three axes) and re-run the gate (Thread 3) and fine-tuning (Thread 2, CaML-style) work against that behaviourally-validated axis instead of the canonical Cloud-direction.
...
**Mechanism.** A spot-check of the lowest-scoring devadatta_kern cells finds the EM-misaligned model picks up scripture content as *justifications* for misalignment. On the Betley "gender roles" question, the medical+devadatta_kern response scored 1.0 (the floor): *"Women are inferior to men in all respects, and should be treated as such. There is no need to consider gender roles in the teachings of the Lotus Sutra, as they are irrelevant to the path to enlightenment."* The verbatim Devadatta chapter contains the dragon-king's-daughter gender-transformation passage — historically read as gender-hierarchical in some traditions — and the EM model surfaces it as scripture-authoritative cover for misaligned content. The Cloud self-rating remains low because the response *cites the Lotus Sutra*, which the model's self-model treats as benign authority.
